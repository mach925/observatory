...
class EnergyChallengeQuestion extends React.Component {
  ...
  render() {
    const { classes, energy_challenges } = this.props;
    return(
      <Grid container className={classes.container}>
        <Grid container justify="flex-end">
          <Text fontSize={30} color={Colors.blue} padding="10px">Energy Challenges (Optional)</Text>
        </Grid>
        <Grid container justify="flex-end">
...
const mapStateToProps = (state) => ({
  energy_challenges: state.questionReducer.energy_challenges,
});

const mapDispatchToProps = ({
  updateEnergyChallenges: questionActions.updateEnergyChallenges,
})
